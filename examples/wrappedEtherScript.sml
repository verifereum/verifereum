...
Definition deployed_accounts_def:
  deployed_accounts = SND (FST (THE deploy_tx_result))
End

val deployed_accounts_pre_def = cv_auto_trans_pre deployed_accounts_def;

Theorem deployed_accounts_pre[cv_pre]:
  deployed_accounts_pre
Proof
  rw [deployed_accounts_pre_def]
  \\ CONV_TAC (PATH_CONV "rlr" cv_eval) \\ rw []
QED
...
val keys = let
  fun foo tm = let
    val (most, last) = finite_mapSyntax.dest_fupdate tm
    val (k, v) = pairSyntax.dest_pair last
    val cont = foo most
    in (k, numSyntax.int_of_term k) :: cont end
    handle _ => []
  in
    parsed_contract_code_eq |> concl |> rhs |> foo
      |> sort (fn (_,a) => fn (_,b) => a < b)
      |> map #1
  end;

val FLOOKUP_parsed_contract_code_thms =
  map (fn e => cv_eval “FLOOKUP parsed_contract_code ^e”) keys;

Theorem FLOOKUP_parsed_contract_code_0 =
  cv_eval “FLOOKUP parsed_contract_code ^(numSyntax.term_of_int 0)”;

Definition static_call_name_tx_def:
  static_call_name_tx = <|
    from := 0x4F26FfBe5F04ED43630fdC30A87638d53D0b0876w
  ; to := SOME 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2w
  ; data := HD abi_4bytes
  ; nonce := 447
  ; value := 0
  ; gasLimit := 1500000
...
val static_call_name_tx_pre_def = cv_auto_trans_pre static_call_name_tx_def;

Theorem static_call_name_tx_pre[cv_pre]:
  static_call_name_tx_pre
Proof
  rw [static_call_name_tx_pre_def, abi_4bytes_def, MAP_TAKE_4_Keccak_256_string_def, abi_signatures_def]
QED


Definition static_call_name_block_def:
  static_call_name_block = <|
    baseFeePerGas := 0 (* fake *)
  ; number := 4719569
  ; timeStamp := 1513077455
  ; coinBase := 0x829BD824B016326A401d083B33D092293333A830w
  ...
  ; hash := 0xd6e5f60d6b2367e74cd2aa520dbeb104826c3932eb482cc16e7f7ef5f8f74799w
  ; parentBeaconBlockRoot := 0w (* fake *)
  ; transactions := [static_call_name_tx] (* not true, many others *)
  |>
End
...
Definition static_call_name_result_def:
  static_call_name_result =
    run_transaction_with_fuel 267 T 1 []
      static_call_name_block deployed_accounts static_call_name_tx
End

val () = cv_auto_trans static_call_name_result_def;
...
cv_eval “(FST (FST (THE static_call_name_result))).output”;
EVAL ``MAP (n2w o ORD) "Wrapped Ether": word8 list``
(*
