...
Theorem enc_valid:
  (∀t v extra.
     has_type t v ∧ LENGTH (enc t v) < dimword(:256) ⇒
     valid_enc t (enc t v ++ extra)) ∧
  (∀hl tl ts vs rhds rtls extra.
     has_types ts vs ∧
     hl = head_lengths ts (SUM (MAP LENGTH (TAKE (LENGTH rtls) rhds))) ∧
     tl = SUM (MAP LENGTH rtls) ∧
     LENGTH (enc_tuple hl tl ts vs rhds rtls) < dimword(:256) ⇒
   let result = enc_tuple hl tl ts vs rhds rtls;
       ...
       hds = DROP hd_len result
   in valid_enc_tuple ts (bs ++ extra) (hds ++ extra) ∧
      (∀n t. ts = REPLICATE n t ⇒
...
Proof
  ...
  (* Bytes NONE case *)
  \\ conj_tac
  >- rw[valid_enc_def, LENGTH_word_to_bytes, TAKE_APPEND, iffRL SUB_EQ_0,
        TAKE_LENGTH_TOO_LONG, LENGTH_REPLICATE, ceil32_def, DROP_APPEND,
        DROP_LENGTH_TOO_LONG, LENGTH_TAKE_EQ]
  (* String case *)
  \\ conj_tac
  >- rw[valid_enc_def, LENGTH_word_to_bytes, TAKE_APPEND, iffRL SUB_EQ_0,
        TAKE_LENGTH_TOO_LONG, LENGTH_REPLICATE, ceil32_def, DROP_APPEND,
  ...
  \\ conj_tac
  >- (
    Cases \\ gvs[LENGTH_word_to_bytes, TAKE_APPEND, TAKE_LENGTH_TOO_LONG]
    \\ rw[valid_int_bound_def, valid_fixed_bounds_def]
    \\ qmatch_asmsub_rename_tac`v9 < 2 ** m`
    \\ `v9 < 2 ** 256`
  ...
  >- (
    ...
    \\ qmatch_asmsub_rename_tac`m ≤ 256`
    \\ (sg `INT_MIN (:256) ≤ ii ∧ ii ≤ INT_MAX (:256)`
        >- (irule int_bits_bound_256 \\ qexists_tac `m`
