...
(* inc_pc_or_jump never returns INR NONE: its only exit paths are
   INL (success via return/set_current_context) or
   INR (SOME InvalidJumpDest) (via assert failure) or
   INR (SOME Impossible) (via empty contexts in get_current_context). *)
Theorem inc_pc_or_jump_not_INR_NONE:
  inc_pc_or_jump op s = (r,s') ==> r <> INR NONE
Proof
  rw[inc_pc_or_jump_def, bind_def, get_current_context_def,
     set_current_context_def, return_def, fail_def, assert_def]
  >> qpat_x_assum `inc_pc_or_jump op s = _` mp_tac
  >> CASE_TAC >> CASE_TAC >> rw[]
QED

(* When the inner step computation (the body of `step` before handle)
   returns (INR NONE, r'), storage is preserved.  This holds because
   INR NONE can only arise from step_inst returning INR NONE directly
   (via Stop -> finish, Return -> step_return T -> finish, or
   SelfDestruct -> finish), and step_inst_inr_preserves_storage covers
   all step_inst INR cases.  The inc_pc_or_jump branch cannot produce
   INR NONE (see inc_pc_or_jump_not_INR_NONE). *)
Theorem inner_step_INR_NONE_preserves_storage:
  (do context <- get_current_context;
   ...
   od) s = (INR NONE, r') /\ s.contexts <> [] ==>
  (!a. (lookup_account a r'.rollback.accounts).storage =
       (lookup_account a s.rollback.accounts).storage)
Proof
  rpt strip_tac
  >> fs[bind_def, get_current_context_def, return_def, fail_def]
  >> Cases_on `s.contexts` >> fs[]
  >> PairCases_on `h`
  >> simp[FST_PAIR, SND_PAIR]
  >> Cases_on `LENGTH h0.msgParams.code <= h0.pc`
  >- (
    (* out of code: step_inst Stop returned INR NONE *)
    `step_inst Stop s = (INR NONE,r')` by fs[]
    >> drule step_inst_inr_preserves_storage
    >> simp[lookup_storage_def, FUN_EQ_THM])
  >> Cases_on `FLOOKUP h0.msgParams.parsed h0.pc`
  >- (
    (* FLOOKUP NONE: step_inst Invalid = fail InvalidOpcode -> INR (SOME _) *)
    fs[step_inst_def])
  >> fs[bind_def]
  >> Cases_on `step_inst x s` >> Cases_on `q`
  >- (
    (* step_inst returned INL: then inc_pc_or_jump must return INR NONE *)
    metis_tac[inc_pc_or_jump_not_INR_NONE, pair_CASES])
  >- (
    (* step_inst returned INR NONE *)
    rename1 `step_inst x s = (INR e,r')`
    >> Cases_on `e` >> fs[]
    >> drule step_inst_inr_preserves_storage
    >> simp[lookup_storage_def, FUN_EQ_THM])
QED
